...
const filtradoStreamers = () => {
    const busquedaStreamers = input.value.toLowerCase(); 
    const filtrador = streamers.filter(streamer =>
        streamer.name.toLowerCase().includes(busquedaStreamers)
    );
    console.log(filtrador); 
};

input.addEventListener('input', filtradoStreamers);